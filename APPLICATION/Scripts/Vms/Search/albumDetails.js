define(["ko", "pubSub", "Modules/dal", "Vms/Extensions/Routing", "Vms/Extensions/Tab"],
    function (ko, pubSub, dal, RoutingExtension, TabExtension) {
        ...
        function initUi() {
            ...
            $("#albumDetailsVm").on("mouseenter", '.albumBlock', function () {
                $(this).find('.albumLikeArea').fadeIn(global.albumHover);
                $(this).find('.albumCoverHover').animate({
                    'marginTop': '-31px'
                });
            });

            $("#albumDetailsVm").on("mouseleave", '.albumBlock', function () {
                $(this).find('.albumLikeArea').fadeOut(global.albumHover);
                $(this).find('.albumCoverHover').animate({
                    'marginTop': '60px'
                });
            });
        ...
        function AlbumDetailsVm() {
            ...
            // Data
            RoutingExtension(self, "album", "Музыка");
            TabExtension(self);
